...
interface CommunityProposalEmailPayload {
  group_name: string;   // 제안자(기관)
  beach_name: string;    // 해변명
  meet_addr?: string;
  title: string;
  description?: string;
...
export function RsvCommunityProposalEmail({ data }: { data: CommunityProposalEmailPayload }) {
  const labelStyle = {
    ...
    <Html>
      <Section style={{
        ...
        <Text style={{ fontSize: '14px', marginBottom: '16px' }}>
          안녕하세요, 사무국 담당자님.<br />
          아래와 같이 커뮤니티 해변정화활동을 신청합니다. 승인 요청드립니다.
        </Text>
        ...
        <Section style={{
          ...
          <Text style={{ fontSize: '0' }}>
            ...
            <span style={valueStyle}>{formatSmartDate(data.startdate).date + " " + formatSmartDate(data.startdate).time + " " + formatSmartDate(data.startdate).relative}</span>
          </Text>
          <Text style={{ fontSize: '0' }}>
            <span style={labelStyle}>종료일</span>
            <span style={valueStyle}>{formatSmartDate(data.enddate).date + " " + formatSmartDate(data.enddate).time + " " + formatSmartDate(data.enddate).relative}</span>
          </Text>
          <Text style={{ fontSize: '0' }}>
        ...
        <Text style={{
          ...
          ※ 본 메일은 커뮤니티 해변정화활동 승인 요청을 위해 발송되었습니다.<br />
