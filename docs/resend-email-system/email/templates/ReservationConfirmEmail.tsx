    ...
    <Html>
      <Section style={{ padding: '24px', fontFamily: 'sans-serif', backgroundColor: '#f9fafb', color: '#1f2937' }}>
        ...
        <Text style={{ fontSize: '14px', marginBottom: '16px' }}>
          {data.partner_group_name} 에서 아래와 같이 확정 일정을 회신드립니다.<br />
        </Text>
        <Section style={{ backgroundColor: '#fff', padding: '12px 20px', borderRadius: '6px', border: '1px solid #e5e7eb', marginBottom: '20px' }}>
          ...
          <Text><b>확정일정</b>: {formatSmartDate(data.confirmed_slot.startdate).date} {formatHM(data.confirmed_slot.startdate)} ~ {formatHM(data.confirmed_slot.enddate)} - {formatSmartDate(data.confirmed_slot.startdate).relative}</Text>
        ...
        <Text style={{ fontSize: '12px', color: '#6b7280', marginTop: '24px', lineHeight: '1.5' }}>
          ※ 본 메일은 확정된 해변정화 일정 안내를 위해 발송되었습니다.<br />
