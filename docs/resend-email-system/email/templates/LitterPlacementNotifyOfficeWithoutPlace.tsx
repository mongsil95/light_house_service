...
export function LitterPlacementNotifyOfficeWithoutPlace({
  groupName,
  ...
  return (
    <Html>
      ...
      >
        ...
        >
          해양폐기물 적치(임시/자율) 신고 알림
        </Heading>
        <Text style={{ marginBottom: "10px", color: "#0e7490" }}>
          해변에 공식 적치 장소가 없어 임시(자율) 적치 위치가 접수되었습니다.
          <br />
        ...
        >
          <Text>
            <b>활동명:</b> {activityTitle || "이름 없음"}
          </Text>
          <Text>
            <b>정화기관:</b> {groupName || "이름 없음"}
          </Text>
          <Text>
            <b>활동일시:</b> {formatSmartDate(startDate).date} {formatSmartDate(startDate).time} ~{" "}
            {formatSmartDate(endDate).date} {formatSmartDate(endDate).time}{" "}
          </Text>
          <Text>
            <b>해변:</b> {beachName}
          </Text>
          <Text>
            <b>[자율]실제 적치 위치(GPS):</b>{" "}
            {dumpedKakaoLink ? (
              <a href={dumpedKakaoLink} target="_blank" style={{ color: "#10b981" }}>
          ...
          <Text>
            <b>지자체:</b> {municipalGroupName || "이름 없음"}
          </Text>
          <Text>
            <b>담당자:</b> {municipalManagerName || "이름 없음"}
          </Text>
          <Text>
            <b>이메일:</b> {municipalManagerEmail || "이메일 없음"}
          </Text>
          <Text>
            <b>내선:</b> {municipalManagerTel || "내선 없음"}
          </Text>
          <Text>
            <b>모바일:</b> {municipalManagerPhone || "없음"}
          </Text>
        </Section>
        {/* 사진 미리보기 */}
