...
const handleRoleChange = (role: UserRole) => {
  ...
  router.push(`/lighthouse?${params.toString()}`);
};
```
    ...
    <div className="flex items-center gap-2 p-2 bg-white border border-gray-200 rounded-lg">
      {ROLES.map((role) => (
        <button
          ...
          className={cn(
            ...
            currentRole === role.id ? "bg-blue-500 text-white" : "hover:bg-gray-100 text-gray-600"
          )}
        >
          <span>{role.icon}</span>
      ...
      <button
        ...
        className={cn(
          ...
          currentRole === "all" ? "bg-gray-200 text-gray-900" : "hover:bg-gray-100 text-gray-500"
        )}
      >
        전체보기
      </button>
...
```tsx
// app/lighthouse/for-kwonsun/guides/page.tsx
...
<RoleToggle className="hidden md:flex" currentRole={currentRole} onRoleChange={onRoleChange} />;
```
